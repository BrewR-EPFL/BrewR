...
fun fetchNearbyCoffeeShops(context: Context, currentLocation: LatLng, radius: Double = 3000.0,onSuccess: (List<JSONObject>) -> Unit) {
    ...
    if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) ==
        ...
        placesClient
            ...
            .addOnSuccessListener { response ->
                // Process the response to retrieve place details
                val coffeeShops = response.places.map { place ->
                    JSONObject().apply {
                        put("id", place.id)
                        ...
                        // Handle reviews
                        put("reviews", place.reviews?.map { review ->
                            JSONObject().apply {
                        ...
                        put("photos", place.photoMetadatas?.map { photoMetadata ->
                            photoMetadata
                        } ?: JSONArray())
                    }
                }
                ...
                //saveToFile(context,"coffee_shops.json", coffeeShops)
...
// private fun saveToFile(context: Context, fileName: String, data: List<JSONObject>) {
...
//    }
//    Log.d("FileSave", "Data saved to ${file.absolutePath}")
// }
